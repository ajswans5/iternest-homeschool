...
const entryPoint = path.join(appRoot, 'scripts', 'curriculumPipelineRunner.ts');
const tempDir = path.join(appRoot, 'node_modules', '.tmp');
const outfile = path.join(tempDir, `iternest-curriculum-pipeline-${Date.now()}.mjs`);
...
if (!outputChunk) {
  throw new Error('Pipeline runner bundle did not produce an executable chunk.');
}
